...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['import_file'])) {
    ...
    foreach ($xml as $item) {
        if ($type == 'websites') {
            $stmt->execute([$item->Web_Address, $item->Web_Name, $item->Web_Login, $item->Web_Password]);
        } elseif ($type == 'banks') {
            $stmt->execute([$item->Bank_Name, $item->Bank_CardNum, $item->Bank_ValidThru, $item->Bank_CardHolder, $item->Bank_Cvv, $item->Bank_CardType, $item->Bank_Pin]);
        }
    }
